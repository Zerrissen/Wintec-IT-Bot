...
const { getTopUsers } = require("../../db/user");
...
module.exports = {
  ...
  async execute(interaction) {
    const embed = new EmbedBuilder().setColor(0x0f4a00).setTitle("Wizard Point Leaderboard");

    const leaderboard = await getTopUsers();

    for (let i = 1; i-1 < leaderboard.length; i++) {
      embed.addFields({
        name: `#${String(i)} @${leaderboard[i-1].username}`,
        value: `${String(leaderboard[i-1].wizardPoints)} points`,
      });
    }
    ...
    //     while (position < 10 && counter < leaderboard.length) {
    //       //counter < leaderboard.length prevents reading past the number of entrees that exist
    ...
    //         .catch((error) => {
    //           console.log(
    //             chalk.red(`[API] Cannot fetch user ID ${record.userID}! No longer in server. Error code: ${error.code}`)
    //           );
    //         });

    //       const memberName = member?.user?.username || "Unknown User"; //Retrives user information
    //       if (memberName != "Unknown User") {
    //         //If the user is a vaild current user, they get added to the leaderboard
